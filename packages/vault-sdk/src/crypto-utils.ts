...
/**
 * Computes a standardized SHA-256 hash of a study payload.
 * If studyPayload is an object, it is stringified first.
 * @param studyPayload The raw study object or string representation.
 */
export async function computeStudyHash(
  studyPayload: string | Record<string, any>
): Promise<string> {
  const inputStr = typeof studyPayload === "string" ? studyPayload : JSON.stringify(studyPayload);
  return sha256Native(inputStr);
}
