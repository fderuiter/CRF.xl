...
import { importOdmXml, OdmImportPackage } from "@crf-xl/taskpane/services/odm-import-service";

import { formatNumber } from "@crf-xl/core/utils/locale-utils";
...
} from "@crf-xl/taskpane/services/speculative-sync-service";
  ...
  const steps: WizardStepDef[] = [
      ...
      content: (
        <>
          <Body1 className={styles.desc}>
            Paste CDISC ODM XML below, or load a file from disk. CRF.xl will parse the supported ODM
            v1 subset and preview the import before any workbook changes are made.
          </Body1>
          <textarea
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            {state.xmlInput.trim().length > 0 && (
              <Text
                style={{ fontSize: tokens.fontSizeBase100, color: tokens.colorNeutralForeground3 }}
              >
    ...
    },
    {
      ...
      content: state.importPackage ? (
        <>
          {state.importPackage.diagnostics.filter((d) => d.severity === "error").length === 0 &&
            state.importPackage.diagnostics.filter((d) => d.severity === "warning").length ===
              0 && (
              <MessageBar intent="success">
                <MessageBarBody>ODM parsed cleanly — no issues found.</MessageBarBody>
              </MessageBar>
            )}
          {state.importPackage.diagnostics
          ...
          <div className={styles.summaryGrid}>
            <div className={styles.summaryItem}>
              <span className={styles.summaryValue}>
                {state.importPackage.summary.actionsCount.addedForms}
              </span>
              <span className={styles.summaryLabel}>Forms detected</span>
            </div>
            <div className={styles.summaryItem}>
              <span className={styles.summaryValue}>
                {state.importPackage.summary.actionsCount.addedCodelists}
              </span>
              <span className={styles.summaryLabel}>Codelists detected</span>
            </div>
            <div className={styles.summaryItem}>
              <span className={styles.summaryValue}>
                {state.importPackage.summary.actionsCount.addedCodelistItems}
              </span>
            ...
            <div className={styles.summaryItem}>
              <span
                ...
                style={{
                  color:
                    state.importPackage.summary.status === "conflicts"
                      ? tokens.colorPaletteRedForeground1
                      : state.importPackage.summary.status === "warnings"
      ...
      content: state.importPackage ? (
        <>
          ...
          <MessageBar intent="warning">
            <MessageBarBody>
              <strong>Non-destructive gate:</strong>{" "}
              {state.importPackage.projection.studyRows.length -
                1 +
                (state.importPackage.projection.formsRows.length - 1) +
                (state.importPackage.projection.codelistRows.length - 1)}{" "}
              data row(s) across 3 system sheets will be written. Existing rows on those sheets will
              be replaced.
            </MessageBarBody>
          </MessageBar>
          <Text className={styles.sectionLabel}>
            _Study ({state.importPackage.projection.studyRows.length - 1} row)
          </Text>
          {renderPreviewTable(state.importPackage.projection.studyRows)}
          <Text className={styles.sectionLabel}>
            _Forms ({state.importPackage.projection.formsRows.length - 1} row(s))
          </Text>
          {renderPreviewTable(state.importPackage.projection.formsRows)}
          <Text className={styles.sectionLabel}>
            _Codelists ({state.importPackage.projection.codelistRows.length - 1} row(s))
          </Text>
          {renderPreviewTable(state.importPackage.projection.codelistRows)}
        </>
      ) : null,
    },
    {
      ...
      content:
        state.importManifest && state.importPackage ? (
          <>
            <MessageBar intent="success">
              <MessageBarBody>
                ODM import complete. {state.importManifest.rowsWritten} data row(s) written to{" "}
                {state.importManifest.sheetsWritten.join(", ")}.
              </MessageBarBody>
            </MessageBar>
            <div className={styles.summaryGrid}>
              <div className={styles.summaryItem}>
                <span className={styles.summaryValue}>{state.importManifest.rowsWritten}</span>
                <span className={styles.summaryLabel}>Rows written</span>
              </div>
              <div className={styles.summaryItem}>
                <span className={styles.summaryValue}>
                  {state.importPackage.summary.actionsCount.warnings}
                </span>
              ...
              <div className={styles.summaryItem} style={{ gridColumn: "1 / -1" }}>
                ...
                >
                  {state.importPackage.study.metadata.protocolId} —{" "}
                  {state.importPackage.study.metadata.studyName}
                </span>
              </div>
            </div>
            ...
            <div className={styles.provenanceBox}>
              ...
              {state.importManifest.provenance.sourceVersion && (
                <div>Version: {state.importManifest.provenance.sourceVersion}</div>
              )}
