...
2. Initialize the workspace: `npm run setup`
...
* **The Task Pane (Frontend):** A React/TypeScript application running inside Excel via `Office.js`. Located in `packages/taskpane/src/components/`, it provides real-time metadata validation, authoring controls, and export triggers. Key views:
...
* **The Clinical Engine (Core):** Located in `packages/core/src/`. Organized into modular subdirectories:
...
1. **Define Types First:** New features start in `packages/core/src/types/`. Add or extend the relevant type definitions in the appropriate modular file (e.g., `clinical.ts` or `enums.ts`).
2. **Build the Parser:** Update `packages/core/src/parser/excel-parser.ts` to read the new workbook columns or sheets into the type model.
3. **Add Validation:** Update `packages/core/src/parser/validator.ts` with any new referential or clinical validation rules.
4. **Implement the Service:** Add any Office.js write-back or external API calls in a new or existing module within `packages/core/src/services/`.
5. **Update the Generators:** Update the relevant builder in `packages/core/src/generators/` (CDISC or DOCX) to support the new metadata.
6. **Wire the UI:** Update or add a component in `packages/taskpane/src/components/views/` to expose the feature in the taskpane.
...
* **Where stable technical contracts live:** Technical interface specs, rules grammar, and schemas are defined in their respective module directories within [packages/](./packages/). When code modifications alter a public signature or type definition, the corresponding spec must be updated in the same PR.
