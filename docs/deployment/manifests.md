...
- **Local developer sideload:** `manifest.xml` (points to `localhost:3000` or dev-tunnel URLs by default, or configured via Webpack).
- **Department/UAT validation:** `manifest.xml` (points to secure UAT sandbox host, configured during build).
- **Centralized rollout in Microsoft 365 Admin Center:** `manifest.xml` (points to canonical production host, configured during build).
