...
import { createTagsFromResource, getDependencyTarget, getUrl, hrTimeToDate, isSqlDB, isSyntheticSource, serializeAttribute, truncateCustomDimensions, } from "./common.js";
...
import { AzureMonitorSampleRate, DEFAULT_BREEZE_DATA_VERSION, DependencyTypes, MicrosoftClientIp, MS_LINKS, } from "./constants/applicationinsights.js";
...
export function readableSpanToEnvelope(span, ikey) {
    ...
    if (baseData && "resultCode" in baseData) {
        ...
        if (dependencyData.resultCode) {
            dependencyData.resultCode = String(dependencyData.resultCode).substring(0, MaxPropertyLengths.TEN_BIT);
        }
        if (dependencyData.data) {
            dependencyData.data = String(dependencyData.data).substring(0, MaxPropertyLengths.THIRTEEN_BIT);
        }
        ...
        if (dependencyData.target) {
            dependencyData.target = String(dependencyData.target).substring(0, MaxPropertyLengths.TEN_BIT);
        }
    }
