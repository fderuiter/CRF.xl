import { StudyDesign, DataType, TranslatedText, CrfItem, EventType } from "../../types/index";
...
<ODM xmlns="http://www.cdisc.org/ns/odm/v1.3" 
  ...
  Object.values(study.forms).forEach((form) => {
    form.itemGroups.forEach((group) => {
      ...
      group.items.forEach((item) => {
        ...
        if (item.showIf) {
          xml += ` CollectionExceptionConditionOID="COND.${item.itemOid}"`;
        }
        xml += `/>`;
      });
      xml += `
  ...
  Object.values(study.forms).forEach((form) => {
    form.itemGroups.forEach((group) => {
      group.items.forEach((item) => {
        if (item.showIf && !processedConditions.has(item.itemOid)) {
          processedConditions.add(item.itemOid);
          xml += `
      <ConditionDef OID="COND.${item.itemOid}" Name="Condition for ${item.itemOid}">
        <Description><TranslatedText xml:lang="${metadata.defaultLanguage || "en"}">Branching script</TranslatedText></Description>
        <FormalExpression Context="Custom">${escapeXml(item.showIf)}</FormalExpression>
      </ConditionDef>`;
        }
      });
    });
