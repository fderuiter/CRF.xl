...
export async function generatePdfBlob(
  ...
  let html = `
    <article class="pdf-page" style="padding: 40px; box-sizing: border-box; page-break-after: always;">
  ...
  for (const [formOid, form] of Object.entries(study.forms)) {
    html += `
      <article class="pdf-page form-page" style="padding: 40px; box-sizing: border-box; position: relative; ${formIndex < Object.keys(study.forms).length - 1 ? "page-break-after: always;" : ""}">
        <header class="clinical-header" style="background-color: #eeeeee; padding: 10px; margin-bottom: 20px; font-weight: bold; font-size: 14px; border: 1px solid #ccc;">
          Protocol ID: ${protocolId} | Form: ${formOid} (${form.formName}) | Subject: ____ | Visit: ____
        </header>
    `;

    form.itemGroups.forEach((group) => {
      html += `<section class="item-group" style="margin-bottom: 20px;">`;
      group.items.forEach((item) => {
        if (isCrfItem(item)) {
          let affordanceText =
            '<span style="width: 20px; height: 20px; border: 1px solid #333; display: inline-block;"></span>'; // Default Checkbox
          if (item.dataType === DataType.INTEGER || item.dataType === DataType.FLOAT) {
            affordanceText =
              '<span style="border-bottom: 1px solid #333; width: 50px; display: inline-block;"></span>';
          } else if (item.dataType === DataType.TEXT) {
            affordanceText =
              '<span style="border-bottom: 1px solid #333; width: 200px; display: inline-block;"></span>';
          } else if (item.vasConfig) {
            affordanceText =
              '<span style="border-bottom: 2px solid #333; width: 300px; display: inline-block; position: relative;"><span style="position: absolute; left: 0; top: -5px; height: 10px; width: 2px; background: #333;"></span><span style="position: absolute; right: 0; top: -5px; height: 10px; width: 2px; background: #333;"></span></span>';
          }

          html += `
            <div class="crf-item" data-oid="${item.itemOid}" style="margin-bottom: 20px; display: flex; align-items: center; position: relative;" role="row">
              <span class="item-label" style="width: 250px; padding-right: 15px;" role="rowheader">${item.name}</span>
              <span class="item-affordance" style="flex-grow: 1;" role="cell">${affordanceText}</span>
            </div>
          `;
        }
      });
