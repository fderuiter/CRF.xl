...
describe('recovery-storage', () => {
    const mockStudy: StudyDesign = {
        ...
        events: [
            { eventOid: 'V1', eventName: 'Visit 1', eventType: 'Scheduled' as any, orderNumber: 1, forms: [] },
            { eventOid: 'V2', eventName: 'Visit 2', eventType: 'Scheduled' as any, orderNumber: 2, forms: [] },
        ],
        forms: {
            F1: { formOid: 'F1', formName: 'Form 1', repeatable: false, itemGroups: [{ items: [{ itemOid: 'A' }, { itemOid: 'B' }] }] } as any,
            F2: { formOid: 'F2', formName: 'Form 2', repeatable: false, itemGroups: [{ items: [{ itemOid: 'C' }] }] } as any,
        },
        codelists: {},
    };

    const issues: ValidationIssue[] = [
        { level: 'Error', message: 'Missing Variable Name.', location: 'F1 > Row 2', sheetName: 'F1', rowIndex: 2 },
