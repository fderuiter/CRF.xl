...
type PersistResult = { saved: true } | { saved: false; reason: 'storage-unavailable' | 'quota-exceeded' | 'unknown' };
...
function isQuotaError(error: unknown): boolean {
    return !!error && typeof error === 'object' && 'name' in error && (error as { name?: string }).name === 'QuotaExceededError';
}
...
export function summarizeStudyDesign(study: StudyDesign): StudyDesignSummary {
    ...
    const variableCount = forms.reduce(
        (total, form) => total + (form.itemGroups ?? []).reduce((groupTotal, group) => groupTotal + (group.items?.length ?? 0), 0),
        0,
    );
    return {
...
export function summarizeValidation(issues: ValidationIssue[], analyzedAt: number = Date.now()): ValidationSummary {
...
export function persistRecoverySnapshot(snapshot: RecoverySnapshot, storage?: StorageLike): PersistResult {
...
export function readRecoverySnapshot({
    ...
    try {
        ...
        if (!parsed || typeof parsed.savedAt !== 'number' || !parsed.validationSummary || !parsed.studySummary || !Array.isArray(parsed.issues)) {
...
export function hasWorkbookChanged(snapshot?: WorkbookFingerprint, current?: WorkbookFingerprint): boolean {
