 ...
 */
export async function saveNewDictionary(id: string, name: string, items: CodelistItem[]): Promise<void> {
    return await Excel.run(async (context) => {
        ...
        context.workbook.names.add("CodelistDictionary", sheet.getRangeByIndexes(1, 0, finalRowCount - 1, 1));
