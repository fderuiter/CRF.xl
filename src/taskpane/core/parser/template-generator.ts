...
export async function initializeWorkbook(): Promise<void> {
    return await Excel.run(async (context) => {
        ...
        const controlConfigs = [
            { name: "_Study", headers: ["Protocol ID", "Study Name", "Version", "Default Language"], data: [["PROT-001", "Matrix Clinical Trial", "1.0", "en-US"]] },
            { name: "_Forms", headers: ["Form OID", "Form Name", "Repeating", "Page Layout"], data: [["DEMO", "Demographics", "No", "Portrait"], ["VS", "Vital Signs", "Yes", "Portrait"]] },
            { name: "_Schedule", headers: ["Form OID", "Visit 1 (Day 0)", "Visit 2 (Day 14)", "Visit 3 (Day 28)"], data: [] }, // Populated dynamically via formula
            { name: "_Codelists", headers: ["Codelist ID", "Codelist Name", "Coded Value", "Decode"], data: [["GENDER", "Gender", "M", "Male"], ["GENDER", "Gender", "F", "Female"]] }
        ];
        ...
        for (const config of controlConfigs) {
            ...
            if (sheet.isNullObject) { sheet = sheets.add(config.name); } 
...
async function syncRegistryInternal(context: Excel.RequestContext): Promise<void> {
    ...
    for (let i = 1; i < rowCount; i++) {
        ...
        if (crfSheet.isNullObject) {
            ...
            crfSheet.getRange("C3:C1000").dataValidation.rule = { list: { inCellDropDown: true, source: "Text,Integer,Float,Date,Time,Datetime,Boolean,Codelist" }};
