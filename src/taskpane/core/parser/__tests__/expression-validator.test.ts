...
describe("CRF.xl Expression static type validation & inference", () => {
  ...
  describe("Type Inference", () => {
    ...
    it("should infer types for resolved variables and qualified identifiers", () => {
      ...
      expect(inferExpressionType(parseRuleExpression("VISIT_1.VS.WT"), variables)).toBe(DataType.FLOAT);
      expect(inferExpressionType(parseRuleExpression("R_001"), variables, knownRules)).toBe(DataType.BOOLEAN);
    ...
    it("should infer types for unary expressions", () => {
      expect(inferExpressionType(parseRuleExpression("-WT"), variables)).toBe(DataType.FLOAT);
      expect(inferExpressionType(parseRuleExpression("!IS_ADULT"), variables)).toBe(DataType.BOOLEAN);
      expect(inferExpressionType(parseRuleExpression("not true"), variables)).toBe(DataType.BOOLEAN);
    });
    ...
    it("should infer Boolean for comparisons and logical operations", () => {
      expect(inferExpressionType(parseRuleExpression("AGE > 18"), variables)).toBe(DataType.BOOLEAN);
      expect(inferExpressionType(parseRuleExpression("SEX == 'M'"), variables)).toBe(DataType.BOOLEAN);
      expect(inferExpressionType(parseRuleExpression("IS_ADULT && true"), variables)).toBe(DataType.BOOLEAN);
    });

    it("should infer types for conditional expressions", () => {
      expect(inferExpressionType(parseRuleExpression("if IS_ADULT then 1 else 0"), variables)).toBe(DataType.INTEGER);
      expect(inferExpressionType(parseRuleExpression("IS_ADULT ? 1.5 : 2"), variables)).toBe(DataType.FLOAT);
      expect(inferExpressionType(parseRuleExpression("if IS_ADULT then 'Adult' else 'Child'"), variables)).toBe(DataType.TEXT);
    });

    it("should infer types for curated functions", () => {
      expect(inferExpressionType(parseRuleExpression("isMissing(WT)"), variables)).toBe(DataType.BOOLEAN);
      expect(inferExpressionType(parseRuleExpression("count(WT, HT)"), variables)).toBe(DataType.INTEGER);
      expect(inferExpressionType(parseRuleExpression("concat(SEX, 'Name')"), variables)).toBe(DataType.TEXT);
      expect(inferExpressionType(parseRuleExpression("sum(AGE, 10)"), variables)).toBe(DataType.INTEGER);
      expect(inferExpressionType(parseRuleExpression("sum(WT, 10)"), variables)).toBe(DataType.FLOAT);
      expect(inferExpressionType(parseRuleExpression("mean(AGE, 10)"), variables)).toBe(DataType.FLOAT);
    });
  });
  ...
  describe("Expression Diagnostics & Safety Checks", () => {
    ...
    it("should catch invalid conditional expression test and branches", () => {
      ...
      expect(diagnostics1.some(d => d.type === "TYPE_ERROR" && d.message.includes("Conditional expression test must be Boolean"))).toBe(true);
      ...
      expect(diagnostics2.some(d => d.type === "TYPE_ERROR" && d.message.includes("branches have incompatible types"))).toBe(true);
    });
    ...
    it("should catch arguments check and unsupported functions", () => {
      ...
      const diagnostics1 = validateExpression(ast1, variables);
      expect(diagnostics1.some(d => d.type === "TYPE_ERROR" && d.message.includes("isMissing"))).toBe(true);

      const ast2 = parseRuleExpression("sum(WT, 'text')"); // invalid argument type
      const diagnostics2 = validateExpression(ast2, variables);
      expect(diagnostics2.some(d => d.type === "TYPE_ERROR" && d.message.includes("sum"))).toBe(true);
