        ...
        if (item.dataType === "Codelist" || item.codelistId) {
          if (!item.codelistId) {
            issues.push({
              ...
              location: `${sheet} > ${item.itemOid || "Row " + row}`,
              rowIndex: row,
              sheetName: sheet,
            });
            ...
            issues.push({
              level: "Error",
              message: `Missing Codelist definition. Codelist ID '${item.codelistId}' not found in _Codelists.`,
              location: `${sheet} > ${item.name || item.itemOid}`,
              rowIndex: row,
              sheetName: sheet,
            });
          }
