...
export class ClinicalValidationPipeline {
  ...
  private validateSdtmDataset(dataset: SdtmDatasetMetadata, isExport: boolean) {
    ...
    if (!dataset.domain || dataset.domain.trim().length === 0) {
      this.issues.push({ level: "Error", message: "SDTM Domain is required.", datasetName: dsName });
    }
    ...
    if (isExport && !dataset.keyVariables?.length) {
      this.issues.push({ level: "Warning", message: "Key variables should be defined for batch export.", datasetName: dsName });
    }
    
    if (dataset.domain && (dataset.domain.length > 8 || !/^[A-Za-z_][A-Za-z0-9_]*$/.test(dataset.domain))) {
      this.issues.push({ level: "Warning", message: "SDTM Domain might not follow SAS dataset naming conventions.", datasetName: dsName });
    }
  }
  ...
  private validateAdamDataset(dataset: AdamDatasetMetadata, isExport: boolean) {
    ...
    if (!dataset.dataset || dataset.dataset.trim().length === 0) {
      this.issues.push({ level: "Error", message: "ADaM Dataset is required.", datasetName: dsName });
    }
    if (!dataset.label || dataset.label.trim().length === 0) {
      this.issues.push({ level: "Error", message: "Label is required.", datasetName: dsName });
    }
  ...
  public validateSdtmVariable(mapping: SdtmMapping, isExport: boolean) {
    ...
    if (hasDomain || hasVar) {
      ...
      if (isExport && hasDomain && hasVar) {
        if (!mapping.core) {
          this.issues.push({ level: "Error", message: `SDTM variable '${mapping.domain}.${mapping.variable}' is missing Core requiredness designation.`, variableName: varName });
        }
        if (!mapping.role) {
          this.issues.push({ level: "Error", message: `SDTM variable '${mapping.domain}.${mapping.variable}' is missing Role designation.`, variableName: varName });
        }
        if (!mapping.sasFieldName) {
          this.issues.push({ level: "Error", message: `SDTM variable '${mapping.domain}.${mapping.variable}' is missing SAS Field Name.`, variableName: varName });
        }
        if (!mapping.sasLabel) {
          this.issues.push({ level: "Error", message: `SDTM variable '${mapping.domain}.${mapping.variable}' is missing SAS Label.`, variableName: varName });
        }
      }
    }
  }
  ...
  public validateAdamVariable(mapping: AdamMapping, isExport: boolean) {
    ...
    if (hasDataset || hasVar) {
      ...
      if (isExport && hasDataset && hasVar) {
        if (!mapping.core) {
          this.issues.push({ level: "Error", message: `ADaM variable '${mapping.dataset}.${mapping.variable}' is missing Core requiredness designation.`, variableName: varName });
        }
        if (!mapping.role) {
          this.issues.push({ level: "Error", message: `ADaM variable '${mapping.dataset}.${mapping.variable}' is missing Role designation.`, variableName: varName });
        }
        if (!mapping.sasFieldName) {
          this.issues.push({ level: "Error", message: `ADaM variable '${mapping.dataset}.${mapping.variable}' is missing SAS Field Name.`, variableName: varName });
        }
        if (!mapping.sasLabel) {
          this.issues.push({ level: "Error", message: `ADaM variable '${mapping.dataset}.${mapping.variable}' is missing SAS Label.`, variableName: varName });
        }
      }
    }
  }
