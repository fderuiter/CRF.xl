...
export const App: React.FC = () => {
    ...
    const handleNavigate = async (issue: ValidationIssue) => {
        // Simple heuristic: if the location contains "Item", we check the Items sheet
        const sheet = issue.location?.includes('Form') ? 'Items' : 'Metadata';
        await navigateToSource(sheet);
    };
        ...
        <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
            <header className="p-4 bg-white border-b border-slate-200 shadow-sm flex items-center justify-between z-10">
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-gradient-to-br from-blue-700 to-blue-900 rounded-xl flex items-center justify-center text-white font-black shadow-lg">C</div>
                    <div>
            ...
            <div className="flex justify-between px-6 py-4 bg-white/50 border-b border-slate-200">
                {[1, 2, 3, 4].map((step) => (
                    <div key={step} className="flex flex-col items-center gap-1.5">
                        ...
                        <span className={`text-[8px] font-bold uppercase tracking-tighter ${currentStep >= step ? 'text-blue-800' : 'text-slate-400'}`}>
            ...
            <main className="flex-grow flex flex-col p-4 gap-4 overflow-hidden relative">
                ...
                <div className="flex flex-col flex-grow overflow-hidden">
                    ...
                    <ValidationLog issues={issues} isProcessing={isProcessing} onNavigate={handleNavigate} />
                </div>
            </main>
        </div>
    );
